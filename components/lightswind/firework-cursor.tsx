...
export type FireworkCursorProps = {
    label?: boolean;
    labelText?: string;
    labelColor?: string;
...
function FireworkCursorBase(props: FireworkCursorProps) {
    const {
    ...
    const appRef = useRef<Particles1CursorApp | null>(null);
    ...
    // Keyed on the joined string so a caller passing a fresh array literal each
    // render does not re-run the sync effect below.
    ...
    useEffect(() => {
        ...
        let app: Particles1CursorApp | null = null;
        let cancelled = false;

        (async () => {
            let particles1: Particles1Cursor | undefined;
            try {
                // Loaded dynamically so a missing/failed dependency degrades to
                // nothing instead of breaking the whole bundle.
                const mod = await import("threejs-components");
                // The build exports each effect by name at the top level. It
                // has neither a default export nor a `cursors` namespace, so
                // the older `default.cursors.particles1` lookup silently
                // resolved to undefined and the cursor never started.
                particles1 = mod?.Particles1Cursor;
            } catch (e) {
                console.warn("Firework Cursor failed to load:", e);
                return;
            }
            if (cancelled || typeof particles1 !== "function") {
                console.warn(
                    "Firework Cursor: Particles1Cursor export not found in threejs-components."
                );
                return;
            }

            try {
                app = particles1(canvas, {
            ...
            if (particles && typeof baseUpdate === "function") {
                particles.update = (arg: Parameters<typeof baseUpdate>[0]) => {
                    ...
                    if (pointer) {
                        ...
                        const nPosition = pointer.nPosition;
                        if (local.over && nPosition) {
                            nPosition.x = local.nx;
                            nPosition.y = local.ny;
                        }
                        if (snapRef.current && nPosition) {
                            ...
                            if (worldSize && target) {
                                target.set(
                                    nPosition.x * worldSize.wWidth * 0.5,
                                    nPosition.y * worldSize.wHeight * 0.5
                                );
                            }
                        }
                    }
                    return baseUpdate.call(particles, arg);
                };
            }
        })();
        ...
        // Only the particle count reallocates the simulation textures, so this
        // effect deliberately re-runs on `density` alone. The remaining props
        // are read once here as seed values and thereafter pushed as uniform
        // writes by the sync effect below — listing them would tear down and
        // rebuild the whole GPU simulation on every slider nudge.
    ...
    }, [color, colors, size, lifetime, bloomStrength]);
...
/**
 * Typed entry point. The untyped default export below is kept for the
 * Originkit preset shape; prefer this one so prop names are checked.
 */
export function FireworkCursor(props: FireworkCursorProps) {
  return <FireworkCursorBase {...(__originkitPresetProps as FireworkCursorProps)} {...props} />;
}

export default FireworkCursor;
